...
function updatePricing() {
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-72px)] md:h-[calc(100vh-70px)] bg-slate-950 w-full overflow-hidden">
      {/* Utility Bar to navigate back since the global layout nav is hidden */}
      <div className="bg-slate-900 border-b border-slate-800 px-3 sm:px-6 py-3 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            <button
            onClick={() => router.back()}
            className="text-slate-400 hover:text-white transition text-xs font-black uppercase tracking-widest bg-slate-800 hover:bg-slate-700 px-4 py-2.5 rounded-lg min-h-[44px] active:scale-95 shrink-0"
            >
            ← <span className="hidden sm:inline">Exit Studio</span><span className="sm:hidden">Exit</span>
            </button>
            <h2 className="text-white font-black uppercase tracking-widest text-xs sm:text-sm truncate">YAYA Studio Engine</h2>
        </div>
      </div>
